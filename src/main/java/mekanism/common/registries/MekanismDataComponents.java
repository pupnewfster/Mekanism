...
public class MekanismDataComponents {
    ...
    private static void registerTooltipAppenders(RegisterTooltipAppendersEvent event) {
        ...
        event.registerComponentAppenderAfterAll(INDUCTION_PROVIDER_TIER, ItemTooltipUtils.createComponentAppender(INDUCTION_PROVIDER_TIER));
        ...
        //TODO - 26.2: Instead of using afterAll for these, just use it for the first one, and then specify the order for mekanism components explicitly so that it is consistent
        // We should also use the debugger to look at what the ordered list ends up being and look at VanillaDataComponentTooltips for what should be in the middle of vanilla data
        // We might need to PR a appenderBetween that lets targetting a before and after element to add two edges to the graph
        event.registerComponentAppenderAfterAll(CONFIGURATION_DATA, ItemTooltipUtils.createComponentAppender(CONFIGURATION_DATA));
        ...
        event.registerComponentAppenderAfterAll(CABLE_TIER, ItemTooltipUtils.createComponentAppender(CABLE_TIER));
        ...
        //TODO - 26.2: Maybe this should be after DataComponents.ENCHANTMENTS ?
        event.registerComponentAppenderAfterAll(MODULE_CONTAINER, ItemTooltipUtils.createSimpleAppender(MODULE_CONTAINER, (_, container, context, _, player, flag, builder) -> {
            if (ItemTooltipUtils.shouldDisplayDetails(context, player, flag)) {
                //TODO - 26.2: If no modules are installed display that as a message when holding shift
                for (IModule<?> module : container.modules()) {
                    ...
                    if (module.getInstalledCount() > 1) {
                        ...
                        builder.accept(MekanismLang.GENERIC_WITH_PARENTHESIS.translateColored(EnumColor.GRAY, data, amount));
                    } else {
                        builder.accept(TextComponentUtil.build(EnumColor.GRAY, data));
                    }
                }
            } else {
        ...
        //TODO - 26.2: The name and skin used to be after energy and before security
        event.registerComponentAppenderAfterAll(ROBIT_NAME, ItemTooltipUtils.createTrivialAppender(ROBIT_NAME, name ->
              MekanismLang.ROBIT_NAME.translateColored(EnumColor.INDIGO, EnumColor.GRAY, name)
        ));
        event.registerComponentAppenderAfter(ROBIT_SKIN, ROBIT_NAME.get(), ItemTooltipUtils.createTrivialAppender(ROBIT_SKIN, skin ->
              MekanismLang.ROBIT_SKIN.translateColored(EnumColor.INDIGO, EnumColor.GRAY, RobitSkin.getTranslatedName(skin))
        ));
        //Modes
        event.registerComponentAppenderAfterAll(BUCKET_MODE, ItemTooltipUtils.createTrivialAppender(BUCKET_MODE, mode ->
              MekanismLang.BUCKET_MODE.translateColored(EnumColor.INDIGO, YesNo.of(mode, true))
        ));
        event.registerComponentAppenderAfter(CONFIGURATOR_MODE, BUCKET_MODE.get(), ItemTooltipUtils.createComponentAppender(CONFIGURATOR_MODE));
        event.registerComponentAppenderAfter(DISASSEMBLER_MODE, CONFIGURATOR_MODE.get(), ItemTooltipUtils.createComponentAppender(DISASSEMBLER_MODE));
        event.registerComponentAppenderAfter(ELECTRIC_BOW_MODE, DISASSEMBLER_MODE.get(), ItemTooltipUtils.createTrivialAppender(ELECTRIC_BOW_MODE, mode ->
              MekanismLang.FIRE_MODE.translateColored(EnumColor.PINK, OnOff.of(mode))
        ));
        //TODO - 26.2: This used to display after stored chemical
        event.registerComponentAppenderAfter(FLAMETHROWER_MODE, ELECTRIC_BOW_MODE.get(), ItemTooltipUtils.createComponentAppender(FLAMETHROWER_MODE));
        event.registerComponentAppenderAfter(FREE_RUNNER_MODE, FLAMETHROWER_MODE.get(), ItemTooltipUtils.createComponentAppender(FREE_RUNNER_MODE));
        //TODO - 26.2: Do we want this to be before attributes?
        event.registerComponentAppenderAfter(JETPACK_MODE, FREE_RUNNER_MODE.get(), ItemTooltipUtils.createComponentAppender(JETPACK_MODE));
        event.registerComponentAppenderAfter(SCUBA_TANK_MODE, JETPACK_MODE.get(), ItemTooltipUtils.createTrivialAppender(SCUBA_TANK_MODE, mode ->
              MekanismLang.FLOWING.translateColored(EnumColor.GRAY, YesNo.of(mode, true))
        ));

        //TODO - 26.2: Move this to data component tooltip positioning, also blocks had this under stats
        event.registerComponentAppenderAfterAll(INVENTORY_FREQUENCY, ItemTooltipUtils.createSimpleAppender(INVENTORY_FREQUENCY, FrequencyAware.TOOLTIP_PROVIDER));
        ...
        event.registerComponentAppenderAfterAll(WASTE_DECAY, ItemTooltipUtils.createSimpleAppender(WASTE_DECAY, (_, _, _, _, _, _, builder) -> {
        ...
        event.registerComponentAppenderAfter(ATTACHED_CHEMICALS, WASTE_DECAY.get(), ItemTooltipUtils.createSimpleAppender(ATTACHED_CHEMICALS, (stack, chemicals, _, _, _, _, builder) -> {
            if (stack.getItem() instanceof IChemicalItem chemicalItem && !chemicals.hasNonEmptyContents()) {
                ...
                boolean displayType = stack.is(MekanismItems.GAUGE_DROPPER);
                if (displayType && !chemicals.hasNonEmptyContents()) {
                ...
                ContainerType.CHEMICAL.addStoredResource(chemicals, builder, MekanismLang.NO_CHEMICAL, EnumColor.ORANGE, stack.get(CHEMICAL_TANK_TIER),
                      displayType ? MekanismLang.CHEMICAL : null);
            }
        }));
        ...
        event.registerComponentAppenderAfter(ATTACHED_FLUIDS, CHEMICAL_TANK_TIER.get(), ItemTooltipUtils.createSimpleAppender(ATTACHED_FLUIDS, (stack, fluids, _, _, _, _, builder) -> {
            if (stack.getItem() instanceof IFluidItem fluidItem && !fluids.hasNonEmptyContents()) {
                ...
                boolean displayType = stack.is(MekanismItems.GAUGE_DROPPER);
                if (displayType && !fluids.hasNonEmptyContents()) {
                ...
                ContainerType.FLUID.addStoredResource(fluids, builder, MekanismLang.NO_FLUID_TOOLTIP, EnumColor.PINK, stack.get(FLUID_TANK_TIER),
                      displayType ? MekanismLang.LIQUID : null);
            }
        }));
        ...
        event.registerComponentAppenderAfter(DETAILS, BIN_TIER.get(), ItemTooltipUtils.createSimpleAppender(DETAILS, TooltipHideType.DESCRIPTION, (_, _, context, _, player, flag, builder) -> {
            if (!ItemTooltipUtils.shouldDisplayDetails(context, player, flag)) {
        ...
        event.registerComponentAppenderAfter(DESCRIPTION, DETAILS.get(), ItemTooltipUtils.createSimpleAppender(DESCRIPTION, TooltipHideType.NONE, (_, description, context, _, player, flag, builder) -> {
            if (ItemTooltipUtils.shouldDisplayDescription(context, player, flag)) {
                ...
                //TODO - 26.2: Test this on a dedicated server. In theory it should never hit this branch so should be fine that the key is only client side
